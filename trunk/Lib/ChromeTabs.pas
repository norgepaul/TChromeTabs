...
type
  ...
  private
    ...
    FOnAnimateStyleTransisiton: TOnAnimateStyleTransisiton;
  ...
  published
    ...
    property OnAnimateStyleTransisiton;
...
begin
  ...
  begin
    ...
    begin
      ...
      FDragTabObject.DragCursorOffset := Point(MouseX - TabControls[HitTestResult.TabIndex].ControlRect.Left + ScrollOffset,
...
procedure TCustomChromeTabs.DeleteDraggedTab;
begin
  if FDragTabObject <> nil then
  begin
    Tabs.Delete(FDragTabObject.DragTab.Index);
...
begin
  ...
  if ((ChromeTabsControl.ControlType = itTab) and
      (ChromeTabsControl.DrawState = dsUnknown)) or
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if GetMaxScrollOffset = ScrollOffset then
...
begin
  Result := FScrollWidth - RectWidth(TabContainerRect) + FOptions.Display.Tabs.TabOverlap;
...
begin
  ...
  begin
    ...
    if OperationID = 2 then
      Tabs.Delete(FMouseDownHitTest.TabIndex);
  end;
  ...
  begin
    ...
    begin
      ...
      NewTabControl.SetPosition(Rect(NewTabLeft,
                                     ...
                                     NewTabLeft + FOptions.Display.Tabs.MinWidth,
...
begin
  ...
  case FScrollDirection of
    mdsLeft: ScrollOffset := ScrollOffset - FOptions.Scrolling.ScrollStep;
    mdsRight: ScrollOffset := ScrollOffset + FOptions.Scrolling.ScrollStep;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Tabs.Delete(HitTestResult.TabIndex);
...
begin
  ...
  if (FOptions.Display.AddButton.Visibility in [avRightFloating, avRightFixed]) then
  begin
...
procedure TCustomChromeTabs.RepositionTabs;
  ...
  begin
    ...
    begin
      ...
      DragTabControl.SetPosition(Rect(CursorPos.X - FActiveDragTabObject.DragCursorOffset.X,
                                      ControlRect.Top + FOptions.Display.Tabs.OffsetTop,
                                      CursorPos.X - FActiveDragTabObject.DragCursorOffset.X + TabWidth,
    ...
    begin
      ...
      begin
        ...
        TabControl.SetPosition(Rect(TabLeft,
                                    FOptions.Display.Tabs.OffsetTop,
                                    TabLeft + TabWidth + FOptions.Display.Tabs.TabOverlap,
                                    ClientHeight - FOptions.Display.Tabs.OffsetBottom),
                               (aeTabMove in FOptions.Animation.AnimationMovement) and
                               (not HasState(stsResizing)));
...
begin
  try
    ...
    if (not FOptions.Scrolling.Enabled) or
       (not ScrollingActive) and // FAdjustedTabWidth > FOptions.Display.Tabs.MinWidth) and
       (FOptions.Display.AddButton.Visibility = avRightFloating) then
    begin
      // Reposition the add button to the end to the tabs
      if GetLastVisibleTabIndex(Tabs.Count - 1) = -1 then
        AddButtonLeft := 0
      else
        AddButtonLeft := TabControls[GetLastVisibleTabIndex(Tabs.Count - 1)].DestinationRect.Right;

      if AddButtonLeft < FAddButtonControl.ControlRect.Left then
...
begin
  Result := Width - 2; { TODO : Why is this adjustment required? }
end;

procedure TCustomChromeTabs.CorrectScrollOffset;
var
...
begin
  ...
  begin
    ...
    begin
      if (X <= FTabContainerRect.Right) and
         (X >= FTabContainerRect.Right - FOptions.Scrolling.DragScrollOffset) then
      begin
        ScrollTabs(mdsRight);
      end else

      if (X >= FTabContainerRect.Left) and
         (X <= FTabContainerRect.Left + FOptions.Scrolling.DragScrollOffset) then
      begin
        ScrollTabs(mdsLeft);
      end
      else
...
function TCustomChromeTabs.CanvasWidth: Integer;
begin
  //Result := TabContainerRect.Right;

  //if FOptions.Layout.AddButton.Visible then
  //  Result := Result + FOptions.Layout.AddButton.Width + FOptions.Layout.AddButton.Offsets.Horizontal + FOptions.Display.Tabs.TabOverlap;

  //if Result < ClientWidth then
  Result := ClientWidth;
end;
...
var
  ...
  begin
    ...
    if FOptions.Display.TabContainer.OverlayButtons then
      TabCanvas.SetClip(RectToGPRectF(Rect(FOptions.Display.Tabs.OffsetLeft,
                                          FOptions.Display.Tabs.OffsetTop,
...
begin
  ...
  begin
    ...
    try
      ...
      // Set the canvas size
      FCanvasBmp.Width := CanvasWidth;
      FCanvasBmp.Height := ClientHeight;

      FBackgroundBmp.Width := CanvasWidth;
      ...
      try
        ...
        for i := pred(FTabs.Count) downto 0 do
          if (Tabs[i].Visible) and
             ...
             (TabControls[i].ControlRect.Right >= FScrollOffset) and
             (TabControls[i].ControlRect.Left <= CorrectedClientWidth + FScrollOffset) then
        ...
        if (ActiveTabIndex <> -1) and
           ...
           (TabControls[ActiveTabIndex].ControlRect.Right >= FScrollOffset) and
           (TabControls[ActiveTabIndex].ControlRect.Left <= CorrectedClientWidth + FScrollOffset) then
...
begin
  Result := Rect(ARect.Left - FScrollOffset,
                 ARect.Top,
                 ARect.Right - FScrollOffset,
                 ARect.Bottom);
end;
...
begin
  Result := Rect(ALeft - FScrollOffset,
                 ATop,
                 ARight - FScrollOffset,
                 ABottom);
end;
...
begin
  ...
  GPImage := BitmapToGPImage(FCanvasBmp);
  try
    for i := low(Targets) to high(Targets) do
    begin
      // Erase the background (copy the real background to the canvas)
      // We need to do this if we're transparent or the background has is transparent
      {$IFDEF DELPHI2006_UP}
      if (FOptions.Display.TabContainer.TransparentBackground) or
         (FLookAndFeel.TabsContainer.StartAlpha < 255) then
        Targets[i].DrawImage(GPImage, 0, 0);
      {$ENDIF}
